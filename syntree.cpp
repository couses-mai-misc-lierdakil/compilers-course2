...
bool operator==(const NodeExp &a, const NodeExp &b) {
  return a.opType == b.opType && a.op1 == b.op1 && a.op2 == b.op2;
}
...
template <> inline std::size_t hash_combine<>(const std::size_t &s) {
  return s;
}

template <typename T> std::size_t myhash(const T &v) {
  std::hash<T> h;
  return h(v);
}

template <typename... Ts> std::size_t hash_combine_calc(const Ts &... args) {
